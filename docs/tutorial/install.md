...
`uv` can run the Globus Registered API CLI without separately installing it.
...
```shell
uv tool run --from='globus-registered-api' gra --help
```
...
> In contrast, the ENTIRE `uv tool run --from='globus-registered-api' gra`
