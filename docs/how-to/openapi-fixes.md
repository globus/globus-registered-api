...
The document below shows an example OpenAPI document for reference.
It is annotated with numbers that correspond with suggestions for fixes
that you may need to make when resolving problems.
...
However, if the scope information is malformed or otherwise incorrect,
you may need to edit the OpenAPI document to fix the scope information.
